...
mkdir Outlines
mkdir MorphParam
cp /home/shrestha/projects/def-stoeber/shrestha/MorphologicalParameters/Scripts/imagej_roi_converter_CC_ArchiveCanadaTimeSeries.py .

# 5) Login node and Compute Node (virtual): Create archive of Outlines and save it in compute node $SLURM_TMPDIR
cd /home/shrestha/projects/def-stoeber/shrestha/Cellpose/OutlinesCanada/TimeSeries/
...
if (( $SLURM_ARRAY_TASK_ID >= 1 && $SLURM_ARRAY_TASK_ID <= 30 )); then
  ...
  command="ImageJ-linux64 --headless --console --run $SLURM_TMPDIR/imagej_roi_converter_CC_ArchiveCanadaTimeSeries.py 'UID=\"${uids[$SLURM_ARRAY_TASK_ID - 1]}\" ,dataPath=\"$SLURM_TMPDIR/Outlines/\" , resultsPath=\"$SLURM_TMPDIR/MorphParam/\"'"
  eval "$command"
else
...
tar -cf /home/shrestha/projects/def-stoeber/shrestha/MorphologicalParameters/Canada/TimeSeries/${uids[$SLURM_ARRAY_TASK_ID - 1]}_MorphologicalParameters.tar ${uids[$SLURM_ARRAY_TASK_ID - 1]}
