...
for fInd in range(len(CSIDfull)):
	...
	for fLInd in range(len(fileList)):
		...
		macroImage = "open(\"/home/shrestha/scratch/ImagesCanada/DPC/TimeSeries/" + UID + "/"  + CSIDfull[fInd] + "/" + IDNum +  "_DPC.png\");"
		...
		if not os.path.isdir(resultsDirUID  + CSIDfull[fInd]):
			os.makedirs(resultsDirUID + CSIDfull[fInd])
			print("Now here: Created directory"+ resultsDirUID + CSIDfull[fInd])

		#savefile = resultsPath + UID + "/ShapeParameters_" + UID + "-" + IDNum + ".csv"
		savefile = resultsDirUID + '/' + CSIDfull[fInd] + "/ShapeParameters_" + UID + "-" + IDNum + ".csv"
