...
mkdir Outlines
mkdir MorphParam
cp /home/shrestha/projects/def-stoeber/shrestha/MorphologicalParameters/Scripts/imagej_roi_converter_CC_ArchiveCanada.py .
...
if (( $SLURM_ARRAY_TASK_ID >= 1 && $SLURM_ARRAY_TASK_ID <= 30 )); then
  ...
  command="ImageJ-linux64 --headless --console --run $SLURM_TMPDIR/imagej_roi_converter_CC_ArchiveCanada.py 'UID=\"${uids[$SLURM_ARRAY_TASK_ID - 1]}\" ,dataPath=\"$SLURM_TMPDIR/Outlines/\" , resultsPath=\"$SLURM_TMPDIR/MorphParam/\"'"
  eval "$command"
else
