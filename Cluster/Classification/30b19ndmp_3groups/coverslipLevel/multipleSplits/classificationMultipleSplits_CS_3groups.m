% classificationMultipleSplits_CS_3groups.m
...
% matlab -nodisplay -r "classifierName = \"12CubicSVM\";classificationMultipleSplits_CS"
...
toc
    ...
    nTrainPreBal(Ni, 1) = length(AA_ABetaTrainInd);
    nTestPreBal(Ni, 1) = length(AA_ABetaTestInd);

    ASTrainInd = find(contains(trainG,'AS'));
    ASTestInd = find(contains(testG,'AS'));
    nTrainPreBal(Ni, 2) = length(ASTrainInd);
    nTestPreBal(Ni, 2) = length(ASTestInd);
    ...
    fprintf('\t Training data numbers: AA-ABeta = %d; AS = %d; SCD = %d \n', length(AA_ABetaTrainInd), length(ASTrainInd), length(SCDTrainInd));

    fprintf('\t Testing data numbers: AA-ABeta = %d; AS = %d; SCD = %d \n', length(AA_ABetaTestInd), length(ASTestInd), length(SCDTestInd));
    ...
    nTrainPostBal(Ni, 1) = length(AA_ABetaTrainInd1);
    nTestPostBal(Ni, 1) = length(AA_ABetaTestInd1);

    ASTrainInd1 = find(contains(trainG1,'AS'));
    ASTestInd1 = find(contains(testG1,'AS'));
    nTrainPostBal(Ni, 2) = length(ASTrainInd1);
    nTestPostBal(Ni, 2) = length(ASTestInd1);
    ...
    fprintf('\t Training data numbers after balancing classes: AA-ABeta = %d; AS = %d; SCD = %d \n', length(AA_ABetaTrainInd1), length(ASTrainInd1), length(SCDTrainInd1));

    fprintf('\t Testing data numbers after balancing classes: AA-ABeta = %d; AS = %d; SCD = %d \n', length(AA_ABetaTestInd1), length(ASTestInd1), length(SCDTestInd1));
