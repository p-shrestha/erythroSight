% classificationMultipleSplits77features_CS.m
...
% matlab -nodisplay -r "classifierName = \"12CubicSVM\";classificationMultipleSplits77features_CS"
...
toc
    ...
    %[trainedClassifier, accVal(Ni)] = trainClassifier12CubicSVM(featSelBaltrainDataCN_4groups_30b19ndmp);
    [trainedClassifier, accVal(Ni)] = eval(strcat('trainClassifier',classifierName,'(featSelBaltrainDataCN_4groups_30b19ndmp);'));
    ...
    tic
    labelsGT{Ni} = featSelBaltestDataCN_4groups_30b19ndmp.Group;
    fprintf('\t Testing data .....');
    [labelsPred{Ni},scores{Ni}] = trainedClassifier.predictFcn(featSelBaltestDataCN_4groups_30b19ndmp);
    toc

    %% Calculate the testing accuracy of the classifier for this iteration
    pred = char(labelsPred{Ni});
    iscorrect = pred == (char(featSelBaltestDataCN_4groups_30b19ndmp.Group));
    iscorrect1 = iscorrect(:,2);
    accTest(Ni) = sum(iscorrect1)/(length(featSelBaltestDataCN_4groups_30b19ndmp.Group));
...
fnameFig = strcat('Figures/2_','confusionMatrixMerged_rowNormalized_Coverslip_',classifierName,'.fig');
print(gcf,fnamePNG,'-dpng','-r600');
saveas(gcf,fnameFig);
close;
toc
