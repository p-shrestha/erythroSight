% classificationMultipleSplits_AA_Trait_SCD.m
% Testing classification of data into 3 groups using different models
% ps UBC 2023

% Can be run in linux or the cluster with the following command (example): 
% matlab -nodisplay -r "classifierName = \"12CubicSVM\";classificationMultipleSplits_AA_Trait_SCD"
...
classifierNamesAll = ["01FineTree", "02MediumTree", "03CoarseTree", "06EffLogReg", "07EffLinSVM", "09KernelNaiveBayes", "10LinSVM", "11QuadSVM", "12CubicSVM", "13FineGaussSVM", "14MedGaussSVM", "15CoarseGaussSVM", "16FineKNN", "17MedKNN", "18CoarseKNN", "19CosineKNN", "20CubicKNN", "21WeightedKNN", "22BoostedTree", "23BaggedTree", "24SubspaceDis", "25SubspaceKNN", "26RUSBoostedTree", "27NarrowNeuralNet", "28MedNeuralNet", "29WideNeuralNet", "30BilayeredNeuralNet", "31TrilayeredNeuralNet", "32SVMKernel", "33LogisticRegKernel"];

% Note: The classfierName can be supplied as an input in the cluster
%classifierName = "11QuadSVM";
...
for Ni = 1:1:nIterations
    ...
    for uidI = 1:1:length(trainABeta)
        ...
        for csI = 1:1:length(trainABeta{1,uidI})
            trainDataCN_3groups_30b19ndmp = [trainDataCN_3groups_30b19ndmp; cell2table([trainABeta{1,uidI}{1,csI}, char(group{2})])];
            tableRow = tableRow + 1;
        end
    end
    ...
    for uidI = 1:1:length(trainAS)
        ...
        for csI = 1:1:length(trainAS{1,uidI})
            trainDataCN_3groups_30b19ndmp = [trainDataCN_3groups_30b19ndmp; cell2table([trainAS{1,uidI}{1,csI}, char(group{2})])];
            tableRow = tableRow + 1;
        end
    end
...
columnNames = [morphParamNameC, 'Group'];
    ...
    for uidI = 1:1:length(testABeta)
        ...
        for csI = 1:1:length(testABeta{1,uidI})
            testDataCN_3groups_30b19ndmp = [testDataCN_3groups_30b19ndmp; cell2table([testABeta{1,uidI}{1,csI}, char(group{2})])];
            tableRowTest = tableRowTest + 1;
        end
    end
    ...
    for uidI = 1:1:length(testAS)
        ...
        for csI = 1:1:length(testAS{1,uidI})
            testDataCN_3groups_30b19ndmp = [testDataCN_3groups_30b19ndmp; cell2table([testAS{1,uidI}{1,csI}, char(group{2})])];
            tableRowTest = tableRowTest + 1;
        end
    end
...
toc
    ...
    ABeta_ASTrainInd = find(contains(trainG,'ABeta-AS'));
    ABeta_ASTestInd = find(contains(testG,'ABeta-AS'));
    nTrainPreBal(Ni, 2) = length(ABeta_ASTrainInd);
    nTestPreBal(Ni, 2) = length(ABeta_ASTestInd);

    SCDTrainInd = find(contains(trainG,'SCD'));
    SCDTestInd = find(contains(testG,'SCD'));
    nTrainPreBal(Ni, 3) = length(SCDTrainInd);
    nTestPreBal(Ni, 3) = length(SCDTestInd);

    fprintf('\t Training data numbers: AA = %d; ABeta-AS = %d; SCD = %d \n', length(AATrainInd), length(ABeta_ASTrainInd), length(SCDTrainInd));

    fprintf('\t Testing data numbers: AA = %d; ABeta-AS = %d; SCD = %d \n', length(AATestInd), length(ABeta_ASTestInd), length(SCDTestInd));


    nTrain = length(AATrainInd)+length(ABeta_ASTrainInd)+length(SCDTrainInd);
    nTest = length(AATestInd) + length(ABeta_ASTestInd) + length(SCDTestInd);
    ...
    nMaxTrain = length(ABeta_ASTrainInd);
    nMaxTest = length(ABeta_ASTestInd);
    ...
    ABeta_ASTrainInd1 = find(contains(trainG1,'ABeta-AS'));
    ABeta_ASTestInd1 = find(contains(testG1,'ABeta-AS'));
    nTrainPostBal(Ni, 2) = length(ABeta_ASTrainInd1);
    nTestPostBal(Ni, 2) = length(ABeta_ASTestInd1);

    SCDTrainInd1 = find(contains(trainG1,'SCD'));
    SCDTestInd1 = find(contains(testG1,'SCD'));
    nTrainPostBal(Ni, 3) = length(SCDTrainInd1);
    nTestPostBal(Ni, 3) = length(SCDTestInd1);


    fprintf('\t Training data numbers after balancing classes: AA = %d; ABeta-AS = %d; SCD = %d \n', length(AATrainInd1), length(ABeta_ASTrainInd1), length(SCDTrainInd1));

    fprintf('\t Testing data numbers after balancing classes: AA = %d; ABeta-AS = %d; SCD = %d \n', length(AATestInd1), length(ABeta_ASTestInd1), length(SCDTestInd1));
...
toc
    ...
    %[trainedClassifier, accVal(Ni)] = trainClassifier12CubicSVM(balancedtrainDataCN_3groups_30b19ndmp);
    [trainedClassifier, accVal(Ni)] = eval(strcat('trainClassifier',classifierName,'(balancedtrainDataCN_3groups_30b19ndmp);'));
    ...
    tic
    labelsGT{Ni} = balancedtestDataCN_3groups_30b19ndmp.Group;
    fprintf('\t Testing data .....');
    [labelsPred{Ni},scores{Ni}] = trainedClassifier.predictFcn(balancedtestDataCN_3groups_30b19ndmp);
    toc

    %% Calculate the testing accuracy of the classifier for this iteration
    pred = char(labelsPred{Ni});
    iscorrect = pred == (char(balancedtestDataCN_3groups_30b19ndmp.Group));
    iscorrect1 = iscorrect(:,2);
    accTest(Ni) = sum(iscorrect1)/(length(balancedtestDataCN_3groups_30b19ndmp.Group));
...
fnamePNG = strcat('Figures/1_',classifierName,'_confusionMatrixMerged_Coverslip','.png');
fnameFig = strcat('Figures/1_',classifierName,'_confusionMatrixMerged_Coverslip','.fig');
...
%set(gcf, 'Units', 'Inches', 'Position', [0, 0, 3.5, 2.163], 'PaperUnits', 'Inches', 'PaperSize', [3.5, 2.163]);
%set(findall(gcf,'-property','FontSize'),'FontSize',8)
fnamePNG = strcat('Figures/2_',classifierName,'_confusionMatrixMerged_rowNormalized_Coverslip','.png');
fnameFig = strcat('Figures/2_',classifierName,'_confusionMatrixMerged__rowNormalized_Coverslip','.fig');
print(gcf,fnamePNG,'-dpng','-r600');
saveas(gcf,fnameFig);
close;
toc
...
box on;
%axis padded;
legend(join([group(1) ," (AUROC = ", sprintf('%.2f',aurocMerged(1)), ")"]), join([group(2) ," (AUROC = ", sprintf('%.2f',aurocMerged(2)), ")"]), join([group(3) ," (AUROC = ", sprintf('%.2f',aurocMerged(3)), ")"]), join(["Macro-averaged (AUROC = ", sprintf('%.2f',aurocMerged(1)), ")"]), 'Location', 'southeast'); % Legends with AUC curve
%set(gcf, 'Units', 'Inches', 'Position', [0, 0, 3.5, 2.163], 'PaperUnits', 'Inches', 'PaperSize', [3.5, 2.163]);
%set(findall(gcf,'-property','FontSize'),'FontSize',8)
fnamePNG = strcat('Figures/3_',classifierName,'_ROCcurveMerged_Coverslip','.png');
fnameFig = strcat('Figures/3_',classifierName,'_ROCcurveMerged_Coverslip','.fig');
print(gcf,fnamePNG,'-dpng','-r600');
saveas(gcf,fnameFig);
close;
