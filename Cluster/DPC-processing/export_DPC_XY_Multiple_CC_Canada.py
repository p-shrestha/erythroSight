...
for UIDInd in range(len(UID)):	# Run analysis for all UID listed
	...
	RAWFolder = (pathParent + '/ImagesCanada/' + 'RAW/' + UID[UIDInd] + '/')	# Path of RAW folder (with all raw images)

	# For the case when processed DPC images are saved in the same path of ImagesCanada
	DPCFolder = (pathParent + '/ImagesCanada/' + 'DPC/'+ UID[UIDInd] + '/')	# Path of DPC folder (where processed images will be stored)
