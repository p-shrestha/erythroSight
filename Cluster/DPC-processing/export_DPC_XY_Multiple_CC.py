...
#		'RAW' > UID ('SS-001') > tname ('t02hrs') > CIDfull (CSID_datestamp_timestamp) > [0, 'acquisition parameters.json', 'configurations.xml'] > [i_j_k_BF_LED_matrix_left_half.bmp, i_j_k_BF_LED_matrix_right_half.bmp, ..., coordinates.csv]
# Folder structure in processed DPC image branch:
#		'DPC' > UID ('SS-001') > tname ('t02hrs') > CIDfull (CSID_datestamp_timestamp) > UID_tname_CSID_i_j_k_DPC.png
...
for UIDInd in range(len(UID)):	# Run analysis for all UID listed
	...
	RAWFolder = (pathParent + '/ImagesNepal/' + 'RAW/' + UID[UIDInd] + '/')	# Path of RAW folder (with all raw images)

	# For the case when processed DPC images are saved in the same path of ImagesNepal
	DPCFolder = (pathParent + '/ImagesNepal/' + 'DPC/'+ UID[UIDInd] + '/')	# Path of DPC folder (where processed images will be stored)
	...
	#DPCFolder = ('/home/shrestha/scratch' + '/ImagesNepal/' + '/DPC/' + UID[UIDInd] )	# Path of DPC folder (where processed images will be stored)
