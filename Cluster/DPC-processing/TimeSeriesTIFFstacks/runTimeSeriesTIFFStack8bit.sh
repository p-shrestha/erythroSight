...
cd $SLURM_TMPDIR
cp /home/shrestha/scratch/ScriptsCCScratch/TimeSeriesTIFF8bit/createTIFFstacks8bit.py .
...
if (( $SLURM_ARRAY_TASK_ID >= 1 && $SLURM_ARRAY_TASK_ID <= 30 )); then
  ...
  command="ImageJ-linux64 --headless --console --run $SLURM_TMPDIR/createTIFFstacks8bit.py 'UID=\"${uids[$SLURM_ARRAY_TASK_ID - 1]}\" ,dataPath=\"/home/shrestha/scratch/ImagesCanada/DPC/TimeSeries/TIFFstacks/\" , resultsPath=\"/home/shrestha/scratch/ImagesCanada/DPC/TimeSeries/TIFFstacks8bit/\"'"
  eval "$command"
else
