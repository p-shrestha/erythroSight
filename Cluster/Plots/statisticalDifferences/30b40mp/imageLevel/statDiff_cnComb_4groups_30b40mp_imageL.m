% statDiff_cnComb_4groups_30b40mp_imageL.m
...
%% 1) Read the frequency distribution data (30 bin) or histDataI

% Note: Here, the histDataI (or image-wise) histcount data is selected
...
tic 
fprintf('Reading data - generalInfo.mat and histDataI.mat \n');
...
for i = 1:1:length(group)
    ...
    % Example: AA.hDi = load('../../../1DCharacterization/t02hrs/cnCombined/30b40mp/AA/histDataI.mat');
    ...
    eval(strcat(group{i},".hDi = load('../../../1DCharacterization/t02hrs/cnCombined/30b40mp/",group{i},"/histDataI.mat');"));
...
%% 2) Concatenate data for Canada and Nepal data so that all images for each group are together

tic

%group = ["AA", "ABeta", "AS", "SCD"]; % Combined data from Nepal and Canada; SCD = SBeta + SS
fprintf('Concatenating data for Canada and Nepal data so that all images for each group are together \n');

% histDataI30b40mp{i}{j}{k} includes data for i = donor, j = image, and
% k = morphological parameter. This needs to be adjusted so that all the
% images for a particular group are in the same group in histDataI
% histDataI{i}{k} includes image level data of i = groups, k =
% morphological parameters

% Concatenate data such that all images for one group is stored in the
% i level of histDataI{i}{k}
for i = 1:1:length(group)
    % Store the first set of all images from the first donor
    % Similar to command: histDataI{1} = AA.hDi.histDataI30b40mp;
    eval(strcat('histDataI{',num2str(i),'}=',group{i},'.hDi.histDataI30b40mp{1};'));

    dNumber = eval(strcat('length(',group{i},'.hDi.histDataI30b40mp', ')'));
    for iD = 2:1:dNumber
        % Concatenate the images for the rest of the donors
        % Similar to histDataI{1} = cat(2, histDataI{1},  AA.hDi.histDataI30b40mp{iD});

        eval(strcat('histDataI{',num2str(i),'}=cat(2,histDataI{',num2str(i),'},',group{i},'.hDi.histDataI30b40mp{',num2str(iD),'});'))
...
for i = 1:1:length(group)
    ...
    for k = 1:1:length(morphParamName)
        ...
        for p = 1:1:length(histDataI{1}{1}{1}) % Iterate through all bins
            clear('jTemp');
            jTemp = zeros(1,length(histDataI{i})); 
            
            for j = 1:1:length(histDataI{i}) % Iterate through all files or image frequencies
                jTemp(j) = histDataI{i}{j}{k}(p);
...
for k = 1:1:length(morphParamLegends)
    ...
    for MPtInd = 1:1:length(midBin{k})
        ...
        for i = 1:1:length(group)
            fMPt{i} = zeros(1,length(histDataI{1,i}));

            for j = 1:1:length(histDataI{1,i})
                [ d, ix ] = min( abs( midBin{k}- MPt ) );
                fMPt{i}(j) = histDataI{1,i}{1,j}{1,k}(ix);
            end
        end
